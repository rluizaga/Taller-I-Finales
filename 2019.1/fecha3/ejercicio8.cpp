...
int main() {
    ...
    std::cout << "Conexión establecida con el servidor en " << SERVER_IP << ":" << SERVER_PORT << std::endl;
    ...
    while ((bytesRead = recv(client_socket, buffer, sizeof(buffer) - 1, 0)) > 0) {
        buffer[bytesRead] = '\0'; // Agregar el carácter nulo al final del buffer para asegurar que sea una cadena válida
